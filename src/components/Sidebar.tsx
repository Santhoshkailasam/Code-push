  ...
  return (
    <aside
      className={`${isCollapsed ? 'w-20' : 'w-56'
          ...
          : 'bg-sky-50/50 border-r border-slate-200 text-slate-900 shadow-sm'
    ...
    >
      <div>
        {/* Brand Header */}
        <div
          className={`p-4 ${isCollapsed ? 'flex justify-center' : 'p-6'} border-b ${isDark ? 'border-slate-800/80' : 'border-slate-200/80'
        ...
        <nav className="p-3 space-y-1.5">
          {!isCollapsed && (
            <div
              className={`px-3 py-2 text-[10px] font-bold uppercase tracking-wider font-mono transition-opacity ${isDark ? 'text-slate-500' : 'text-slate-400'
                }`}
            >
              Navigation
            </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative w-full flex items-center ${isCollapsed ? 'justify-center px-0 py-3' : 'space-x-3 px-3.5 py-2.5'
                  } rounded-xl text-sm font-medium transition-all duration-200 ease-out active:scale-[0.97] cursor-pointer ${isActive
                    ? isDark
      ...
      <div
        className={`p-3 border-t ${isDark ? 'border-slate-800/80' : 'border-slate-200/80'
          } space-y-2`}
      >
        <div
          className={`flex items-center ${isCollapsed ? 'justify-center p-2' : 'space-x-2.5 px-3 py-2'
            } rounded-xl border text-xs transition-all duration-300 ${isDark
